...
export const FeaturedItems = () => {
  return (
    <section className="mx-auto max-w-7xl px-4 py-8 md:py-12 lg:py-16 text-[#1c1f58]">
      <div className="mb-8 md:mb-12 flex flex-col items-center justify-center gap-4">
        <h2 className={`max-w-3xl text-2xl md:text-3xl lg:text-4xl font-semibold text-center leading-tight md:leading-10 ${poppins}`}>
          What will you accomplish with BudgetBuddy?
        </h2>
      </div>
      <div className={`${poppins} mb-4 grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6`}>
...
const BounceCard = ({
  ...
  return (
    <motion.div
      ...
      className={`group relative min-h-[250px] md:min-h-[300px] cursor-pointer overflow-hidden rounded-2xl bg-slate-100 p-4 md:p-8 ${className}`}
    >
      {children}
    </motion.div>
  );
...
const CardTitle = ({ children }: { children: ReactNode }) => {
  return (
    <h3 className="mx-auto text-center text-xl md:text-2xl lg:text-3xl font-semibold">
      {children}
    </h3>
  );
};
