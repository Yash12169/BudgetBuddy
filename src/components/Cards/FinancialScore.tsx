...
export default function FinancialScore() {
  ...
  useEffect(()=>{
    if(financials && emergencyFund){
       ...
       const emergencyScore = emergencyFund.emergencyFundStatus.score;
       setTotalScore((FinancialScore+emergencyScore)/2);
    }
  },[financials,emergencyFund])
  useEffect(() => {
      ...
      <div className="flex w-[100%] h-[100%]  ">
        <div className="flex w-[65%] h-[100%] flex-col gap-5">
          ...
          <div className="bg-[#d4d4d4c3] w-[90%] h-[5vh] flex rounded-[15px]">
            <div style={{width:`${width}%`}} className={`bg-[#ec0072] h-[100%] rounded-[15px] w-[${totalScore}%] transition-all duration-1000 ease-in-out`}></div>
          </div>
        </div>
        ...
        <div className={`flex w-[45%] flex-col h-[100%] justify-center gap-5 `}>
            ...
            <div className="flex flex-col">
              <div className="flex gap-2">
                ...
                <div className="text-xs rounded-[15px] px-2 py-1 bg-black text-white">
                  <p className={`${montserrat}`}>
                    <span className="font-semibold">{financials.data.savingScore}</span>
                    <span className="text-[#ffffffaa]">/100</span>
                  </p>
                </div>
              </div>
              <div className={`${montserrat} text-xs`}>
                <p>Weak Score, Medium Impact</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 items-center">
            <div className="rounded-full border-2 border-[#4A9BE6] justify-center items-center flex h-fit p-2">
              <Image src={shield} alt="wallet" height={24} width={24} />
            </div>
            <div className="flex flex-col">
              <div className="flex gap-2">
                <div
                ...
                <div className="text-xs rounded-[15px] px-2 py-1 bg-black text-white">
                  <p className={`${montserrat}`}>
                    <span className="font-semibold">{emergencyFund.emergencyFundStatus.score}</span>
                    <span className="text-[#ffffffaa]">/100</span>
                  </p>
                </div>
              </div>
